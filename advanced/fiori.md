...
Be aware that all Compositions of the draft enabled entity are part of the same draft. Only those entities will get a `CREATE` button in SAP Fiori elements UIs as they are part of the draft. Associated entities on the other side can only be deleted or modified, however here the changes are directly applied instead of being applied once `SAVE` is pressed. 
