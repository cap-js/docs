...
If you want to define the configurations that means all the options in a config file, you can use `--openapi:config-file <JSON_config_filepath>` which accepts a JSON file incorporating all the options of openapi compile. Here, inline Options are given precedence over those defined in the config file.
