...
Vector embeddings are numerical representations that capture important features and semantics of unstructured data - such as text, images, or audio - in a way that similar data is represented by vectors with high similarity and low distance to each other. This vector representation facilitates tasks like similarity search, anomaly detection, recommendations and Retrieval Augmented Generation (RAG) - improving the result of generative AI (GenAI) by augmenting user instructions (prompts) to large language models (LLMs) with relevant information retrieved using a vector datastore such as the [SAP HANA Cloud Vector Engine](https://community.sap.com/t5/technology-blogs-by-sap/sap-hana-cloud-s-vector-engine-announcement/ba-p/13577010) as context to the LLM.

Typically vector embeddings are computed using an embedding model specifically designed to capture important features and semantics of a specific type of data, also the dimensionality of the vector embedding space depends on the embedding model. Unified consumption of embedding models and LLMs across different vendors and open source models is provided via the [SAP Generative AI Hub](https://community.sap.com/t5/technology-blogs-by-sap/how-sap-s-generative-ai-hub-facilitates-embedded-trustworthy-and-reliable/ba-p/13596153).
