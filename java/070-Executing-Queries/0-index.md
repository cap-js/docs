...
permalink: java/query-execution
status: released
...
[CDS Query Language (CQL)]({{java-cql}}) statements can be executed using the `run` method of any [service that accepts CQN queries]({{java-consumption}}#cdsservices):
...
To query [views with parameters]({{cap}}/advanced/hana#views-with-parameters) on SAP HANA, you need to build a select statement and execute it with the corresponding named parameters.
...
To be able to query and lock the data until the transaction is completed, just call a [`lock()`]({{cap}}/java/query-api#write-lock) method and set an optional parameter `timeout`.
...
The [update]({{java-cql}}) operation can be executed as follows:
...
By default, *insert*, *update* and *delete* operations cascade over [compositions]({{cap}}/guides/domain-models#compositions-capture-contained-in-relationships) only. For associations, this can be enabled using the `@cascade` annotation.
...
_❗ Warning_{:.warning-title}
The @cascade annotation is not respected by [foreign key constraints on the database]({{cap}}/guides/databases#for-compositions). To avoid unexpected behaviour you might have to disable a FK constraint with [`@assert.integrity:false`]({{cap}}/guides/providing-services#refs).
{:.warning}

#### Deep Insert / Upsert
{: #deep-insert-upsert}

[Insert]({{cap}}/java/query-api#insert) and [upsert]({{cap}}/java/query-api#upsert) statements for an entity have to include the keys and (optionally) data for the entity's composition targets. The targets are inserted or upserted along with the root entity.
...
The [delete]({{java-cql}}) operation is cascaded along the entity's compositions. All composition targets that are reachable from the (to be deleted) entity are deleted as well.
...
- For the Persistence Service, this means the runtime _attempts_ to execute the write operation on the database view. Whether this execution is possible is [database dependent]({{java}}/persistence-services#database-support).
...
CAP Java doesn't have a dedicated API to execute native SQL Statements. However, when using Spring as application framework you can leverage Spring's features to execute native SQL statements. See [Execute SQL statements with Spring's JdbcTemplate]({{cap}}/java/advanced#jdbctemplate) for more details.


## Query Result Processing
{: #result}

The result of a query is abstracted by the `Result` interface, which is an iterable of `Row`. A `Row` is a `Map<String, Object>` with additional convenience methods and extends [CdsData]({{cap}}/java/data#cds-data).
...
The element names and their types are checked only at runtime. Alternatively you can use interfaces to get [typed access]({{java}}/data#typed-access) to the result data:
...
For the entities defined in the data model, CAP Java SDK can generate interfaces for you through [a Maven plugin]({{java}}/advanced#staticmodel).


### Using Entity References from Result Rows in CDS QL Statements
{:#entity-refs}

For result rows that contain all key values of an entity, you get an [entity reference]({{java}}/query-api#entity-refs) via the `ref()` method. This reference addresses the entity via the key values from the result row.
...
The `rowType` method allows to introspect the element names and types of a query's `Result`. It returns a `CdsStructuredType` describing the result in terms of the [Reflection API]({{java}}/reflection-api):
