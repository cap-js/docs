...
Actions or functions are handled - just like CRUD events - using event handlers. To trigger an action or function on an Application Service an event with the action's or function's name is emitted on it. The CAP Java runtime doesn't provide any default `On` handlers as they are provided for the CRUD events.
...
2. The event handler implementation
