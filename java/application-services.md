...
The CAP Java runtime doesn't provide any default `On` handlers as they are provided for the CRUD events. Actions and functions are therefore implemented through event handlers. For each action or function an event handler of the [`On`](provisioning-api#on) phase should be defined, which implements the business logic and provides the return value of the operation, if applicable. The event handler needs to take care of [completing the event processing](provisioning-api#eventcompletion).
...
2. The event handler registration and implementation
...
Now, the review action can be easily called from custom handler code:
