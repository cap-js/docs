...
### Outbox Event Versions { #outbox-event-versions}
...
To avoid this problem, you can configure the outbox to use an event version that prevents the outbox collectors from using the newer evens. For this purpose, you can set the parameter `cds.environment.deployment.version` in the `application.yaml` file.
::: code-group
```yaml [srv/src/main/resources/application.yaml]
cds:
  environment:
...
Please note that the configured deployment versions must be in ascending order. The messages are only processed by the outbox collector if the event version is less than or equal to the deployment version.
:::
...
To do this, the maven resource filtering configuration in the `srv/pom.xml` must be activated so that the app version placeholder `${project.version}` can be used in the `application.yaml` configuration 
...
And finally, if for some reason you do not want to use a version check for a particular outbox collector, you can switch it off via the outbox configuration:
