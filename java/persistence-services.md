...
7. The H2 uses Java implementation of regular expressions. The matching behaviour is an equivalent of the `Matcher.find()` call for the given pattern.  
...
10. CAP delivers an enhancement to support regular expressions in the SQLite via Java implementation. The matching behaviour is an equivalent of the `Matcher.find()` call for the given pattern.
