...
* Localized entities can be locked only if your query is executed without a locale, as described in the chapter [Modifying Request Context](../event-handlers/request-contexts#modifying-requestcontext). Alternatively the localized element can be removed from the select list (columns), then locking is possible on localized entities.
