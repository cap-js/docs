...
Access control is performed on basis of CQN level according to the CDS model and hence HTTP Verb Tampering attacks are avoided. Also HTTP method override via `X-Http-Method-Override` or `X-Http-Method` headers are not accepted by the runtime.
